...
contract PenaltySystem is AccessControl, ReentrancyGuard {
    ...
    event PenaltySettled(bytes32 indexed caseId, address indexed user, uint256 amount);
     ...
     */
    function applyIdentityFraudPenalty(bytes32 caseId_, address user_)
        external
        ...
        nonReentrant
    {
        ...
        _recordPenalty(caseId_, user_, penalty, PenaltyType.IdentityFraud, false, incentivesPool, true);
    }
    ...
    function applyMassSalePenalty(
        ...
        _recordPenalty(caseId_, user_, penalty, PenaltyType.MassSale, false, incentivesPool, true);
    }

    /**
     * @notice Apply no-show interview penalty (mechanism 14).
     */
    function applyNoShowPenalty(bytes32 caseId_, address user_)
        external
        onlyRole(ENFORCER_ROLE)
        nonReentrant
    {
        ...
        _recordPenalty(caseId_, user_, penalty, PenaltyType.NoShowInterview, false, incentivesPool, true);
    }

    /**
     * @notice Apply educator inactivity penalty (mechanism 25).
     */
    function applyEducatorInactivityPenalty(bytes32 caseId_, address educator_)
        external
        onlyRole(ENFORCER_ROLE)
        nonReentrant
    {
        ...
        _recordPenalty(caseId_, educator_, penalty, PenaltyType.EducatorInactivity, true, incentivesPool, true);
    }
    ...
    function applyPlagiarismPenalty(
        ...
        _recordPenalty(caseId_, offender_, penalty, PenaltyType.Plagiarism, true, destination, false);
    }

    /**
     * @notice Apply recruiter inactivity penalty (mechanism 30).
     */
    function applyRecruiterInactivityPenalty(bytes32 caseId_, address recruiter_)
        external
        onlyRole(ENFORCER_ROLE)
        nonReentrant
    {
        ...
        _recordPenalty(caseId_, recruiter_, penalty, PenaltyType.RecruiterInactivity, true, incentivesPool, true);
    }
    ...
    function settlePenalty(bytes32 caseId_) external nonReentrant {
        ...
        if (p.isPoolDeposit) {
            IIncentivesPool(incentivesPool).deposit(p.amount, "penalty_settlement");
        }

        if (penaltyDebt[msg.sender] == 0 && profileBlocked[msg.sender]) {
            profileBlocked[msg.sender] = false;
    ...
    function _recordPenalty(
        ...
        emit PenaltyApplied(caseId_, user_, penaltyType_, amount_, blockProfile_);
    }
